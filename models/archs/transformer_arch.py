...
class Block(nn.Module):
    ...
    def __init__(self, bert_n_emb, resid_pdrop, bert_n_head, attn_pdrop,
        ...
        self.attn = CausalSelfAttention(bert_n_emb, bert_n_head, attn_pdrop,
                                        resid_pdrop, block_size, sampler)
...
class TransformerLanguage(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.Sequential(*[
            Block(bert_n_emb, resid_pdrop, bert_n_head, attn_pdrop, block_size,
                  sampler) for _ in range(self.n_layers)
        ])
        # decoder head
        self.ln_f = nn.LayerNorm(self.n_embd)
        self.identity_head = nn.Linear(
            self.n_embd, self.codebook_size, bias=False)
